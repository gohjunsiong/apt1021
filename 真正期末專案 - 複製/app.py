...
def create_order(customer_id, item_name, merchant_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        # 插入新訂單到 orders 表
        cursor.execute('''
            INSERT INTO orders (customer_id, item_name, acceptance_status)
            VALUES (?, ?, "待確認")
        ...
        # 插入新訂單到 merchant_orders 表
        cursor.execute('''
            INSERT INTO merchant_orders (order_id, merchant_id, acceptance_status)
            VALUES (?, ?, "待確認")
...
def get_order_details(order_id):
    ...
    try:
        cursor.execute('''
            SELECT orders.*, merchant_orders.merchant_id, merchant_orders.acceptance_status AS merchant_acceptance_status
            FROM orders
            JOIN merchant_orders ON orders.id = merchant_orders.order_id
            WHERE orders.id = ?
        ''', (order_id,))
...
def merchant_reject_order(order_id):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM merchant_orders')
        all_merchant_orders = cursor.fetchall()
        cursor.execute('SELECT * FROM orders')
        all_orders = cursor.fetchall()

        print("All merchant_orders entries:")
        for row in all_merchant_orders:
            print(dict(row))  # 將 row 轉換成字典並打印

        print("All orders entries:")
...
def add_review(order_id):
    ...
    try:
        ...
        # 如果任意字段缺失，返回錯誤
        if not all([rating, comment, reviewed_user_id]):
            print(f"警告: 有字段缺失 - rating: {rating}, comment: {comment}, reviewed_user_id: {reviewed_user_id}")
            return "表單字段錯誤，缺少關鍵資料", 400

        # 打印接收到的數據以便於調試
        print(f"Received: order_id={order_id}, rating={rating}, comment={comment}, reviewed_user_id={reviewed_user_id}")
        ...
        return redirect(url_for('orders'))  # 或其他需要重定向的頁面

    except KeyError as e:
        # 打印缺失的字段名稱以便於調試
        print(f"KeyError: 缺失字段: {e}")
        return "表單字段錯誤，缺少關鍵資料", 400
...
def view_reviews(user_id):
    ...
    reviews = cursor.execute('''
        SELECT reviews.rating, reviews.comment, users.username, reviews.created_at, menu.item_name
        FROM reviews
        JOIN users ON reviews.user_id = users.id
