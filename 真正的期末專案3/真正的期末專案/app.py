...
def init_db():
    ...
    conn.execute('''CREATE TABLE IF NOT EXISTS orders (
                ...
                price REAL NOT NULL,  -- 添加 price 字段
...
def place_order(item_id):
    ...
    # 创建订单
    conn.execute('''INSERT INTO orders (customer_id, merchant_id, item_id, status, price)
                    VALUES (?, ?, ?, ?, ?)''',
                 (session['user_id'], item['merchant_id'], item['id'], '待处理', price))
    conn.commit()
    conn.close()
